...
|-------|-------|-------------|
| `POST /api/run` | `name` | **Confined** — `startLocal` → `ensureRunProject` → `workspaceDir` → `workspaceName()`. The eve run-dir basename is a slugged project name. Traversal names fail closed (no spawn, no dir outside the root). |
| `POST /api/materialize`, `/api/archive` | `dir`, `projects` | **Safe by design** — the local introspection root the user points the tool at; the same trusted local path the whole app reads. |
