# agentgem Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A local web UI (on AgentBack) that introspects the coding-agent config (skills, MCP servers, CLAUDE.md) and builds a secret-redacted Gem, exposing `inventory`/`gem` as REST endpoints + MCP tools from one Zod contract.

**Architecture:** Standalone AgentBack hybrid app. `gem-core` (pure TS, ported from `workflow-profiler`) does introspection/build/redaction. A REST controller and an MCP tools class — both delegating to gem-core and sharing zod-v4 wire schemas — expose `GET /api/inventory` + `POST /api/gem`. A static two-pane page (layout B) is served at `/` from `server.expressApp` and is just another client of the API.
...
**Spec:** `docs/superpowers/specs/2026-06-14-agentgem-design.md`
**gem-core source to port:** `/Users/rfeng/Projects/ninemind/workflow-profiler/src/gem/{types,redact,introspect,buildPack}.ts` and `/Users/rfeng/Projects/ninemind/workflow-profiler/tests/gem/{redact,introspect,buildPack}.test.ts`.
...
agentgem/
  ...
  src/
    gem/                       # ported gem-core (pure TS, no zod)
      types.ts redact.ts introspect.ts buildPack.ts
      __tests__/redact.test.ts introspect.test.ts buildPack.test.ts
    schemas.ts                  # zod v4 wire schemas (Inventory, PackRequest, Gem)
    gem.controller.ts          # @api REST: GET /api/inventory, POST /api/gem
    gem.tools.ts               # @mcpServer MCP tools: inventory, gem
    public/index.html           # two-pane page (vanilla, served at /)
    index.ts                    # app bootstrap: REST + MCP + static page + start
    __tests__/gem.controller.test.ts   # supertest against a started app
```
...
git commit -m "chore: scaffold agentgem (AgentBack hybrid, zod v4, build+vitest)"
```

---

### Task 2: Port gem-core (introspect / buildPack / redact) + tests

**Files:** Create `src/gem/{types,redact,introspect,buildPack}.ts` and `src/gem/__tests__/{redact,introspect,buildPack}.test.ts`.

- [ ] **Step 1: Copy the four pure source files verbatim** from the profiler:
```bash
mkdir -p src/gem/__tests__
cp /Users/rfeng/Projects/ninemind/workflow-profiler/src/gem/types.ts       src/gem/types.ts
cp /Users/rfeng/Projects/ninemind/workflow-profiler/src/gem/redact.ts      src/gem/redact.ts
cp /Users/rfeng/Projects/ninemind/workflow-profiler/src/gem/introspect.ts  src/gem/introspect.ts
cp /Users/rfeng/Projects/ninemind/workflow-profiler/src/gem/buildPack.ts   src/gem/buildPack.ts
```
These have **no zod dependency** (pure TS), so they compile unchanged under this project's tsconfig.

- [ ] **Step 2: Copy the three test files** and fix their import depth (tests now live in `src/gem/__tests__/`, so `../../src/gem/X` becomes `../X`):
```bash
cp /Users/rfeng/Projects/ninemind/workflow-profiler/tests/gem/redact.test.ts     src/gem/__tests__/redact.test.ts
cp /Users/rfeng/Projects/ninemind/workflow-profiler/tests/gem/introspect.test.ts src/gem/__tests__/introspect.test.ts
cp /Users/rfeng/Projects/ninemind/workflow-profiler/tests/gem/buildPack.test.ts  src/gem/__tests__/buildPack.test.ts
```
Then in each copied test file, replace the import specifier `../../src/gem/` with `../` (e.g. `import { redactMcpConfig } from "../redact";`). Make NO other change to the test bodies.

- [ ] **Step 3: Run the tests** — `pnpm test`
Expected: PASS — the ported redact (5), introspect (3), buildPack (4) tests all green (compiled to `dist/gem/__tests__/` and run there). `pnpm build` is clean.

- [ ] **Step 4: Commit**
```bash
git add src/gem
git commit -m "feat: port gem-core (introspect/buildPack/redact) + tests from workflow-profiler"
```
...
- [ ] **Step 3: Create `src/schemas.ts`** (zod v4; mirrors `gem/types.ts`)
```typescript
// src/schemas.ts
import { z } from "zod";
...
git commit -m "feat: zod-v4 wire schemas for inventory/gem-request/gem"
```
...
**Files:** Create `src/gem.controller.ts`. Test: `src/__tests__/gem.controller.test.ts`.
...
- [ ] **Step 3: Create `src/gem.controller.ts`**
```typescript
...
import { introspectConfig } from "./gem/introspect.js";
...
export class PackController {
  ...
  async gem(input: { body: z.infer<typeof PackRequestSchema> }): Promise<z.infer<typeof PackSchema>> {
    ...
    return buildPack(inventory, input.body.selection, { name: input.body.name ?? "gem", createdFrom: dir });
  }
}
```
If `tsc` complains that `introspectConfig`'s `ConfigInventory` return type isn't assignable to `z.infer<typeof InventorySchema>` (or `buildPack`'s `Gem` vs `PackSchema`), the schema and the gem-core type have drifted — reconcile `src/schemas.ts` to match `src/gem/types.ts` exactly (do NOT loosen the redaction or change gem-core). If AgentBack's `@get` doesn't accept a `query` schema key in 0.2.2, check `examples/hello-hybrid` / `@agentback/openapi` types in the agentback repo and use the supported form (e.g. read the dir from `input.query` if present, else default); keep the `?dir=` behavior.
...
git commit -m "feat: REST controller — GET /api/inventory, POST /api/gem (redacted)"
```
...
**Files:** Create `src/gem.tools.ts`.

> MCP runtime testing needs an MCP client harness; for this task the gate is `tsc` clean + the tools registering. Functional MCP verification happens in the Task 6 smoke (`/mcp` reachable) and is exercised by the operator's agent. The tool handlers are thin delegations to the already-tested gem-core.

- [ ] **Step 1: Create `src/gem.tools.ts`** (mirror `examples/hello-hybrid` `@mcpServer`/`@tool`)
```typescript
// src/gem.tools.ts
import { homedir } from "node:os";
import { join } from "node:path";
...
export class PackTools {
  ...
  async gem(input: z.infer<typeof PackInput>) {
    const dir = resolveDir(input.dir);
    return buildPack(introspectConfig(dir), input.selection, { name: input.name ?? "gem", createdFrom: dir });
  }
}
```
...
- [ ] **Step 3: Commit**
```bash
git add src/gem.tools.ts
git commit -m "feat: MCP tools (inventory, gem) sharing gem-core + schemas"
```
...
<body>
<header><h1>agentgem</h1><span class="muted">Gem Builder · introspecting <code id="dir">~/.claude</code></span></header>
<main>
  <section class="pane left">
    <div class="bar"><input id="name" type="text" placeholder="gem name" value="gem" /><button id="all" class="ghost">Select all</button></div>
    <div id="inventory">Loading…</div>
  </section>
  <section class="pane right">
    <div class="bar"><strong style="flex:1">gem.json (live)</strong><button id="copy" class="ghost">Copy</button><button id="dl">⬇ Download</button></div>
    <pre id="preview">{}</pre>
...
async function build(){
  ...
  const name = document.getElementById("name").value || "gem";
  const gem = await (await fetch("/api/gem", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ selection, name }) })).json();
  window.__pack = gem;
  document.getElementById("preview").textContent = JSON.stringify(gem, null, 2);
}
...
echo "--- gem (all) ---"; curl -s -X POST localhost:4317/api/gem -H 'content-type: application/json' -d '{"selection":{"all":true},"name":"smoke"}' | grep -o '"name":"smoke"'
echo "--- no secret leak in gem ---"; curl -s -X POST localhost:4317/api/gem -H 'content-type: application/json' -d '{"selection":{"all":true}}' | grep -qiE "ghp_|sk-ant|xox" && echo "FAIL leak" || echo "OK no leak"
kill $SERVER
```
Expected: title present; `/api/inventory` returns JSON with `skills`; gem returns `"name":"smoke"`; "OK no leak". Then `pnpm test` (full suite still green) and `pnpm build` clean.

- [ ] **Step 5: Commit**
```bash
git add src/public/index.html src/index.ts package.json
git commit -m "feat: two-pane gem-builder page + AgentBack bootstrap (REST + MCP + static)"
```
...
- [ ] **Step 2: Verify with the gstack browser** (the operator has gstack). Load `/`, confirm: the left pane lists real skills (72) + any MCP servers + CLAUDE.md; checking a skill updates the right-pane `gem.json` live; the preview shows `<redacted>` for any MCP secret and never a raw secret; "Download" yields a valid `gem.json`. Screenshot before/after a selection.
...
- **Spec coverage:** AgentBack hybrid app (Tasks 1,4,5,6 — spec §1,§2); gem-core ported (Task 2 — §3); zod-v4 wire schemas (Task 3 — §3); REST `GET /inventory` + `POST /gem` (Task 4 — §4); MCP tools `inventory`/`gem` (Task 5 — §4); two-pane page served statically + bootstrap with `/explorer` + `/mcp` (Task 6 — §4,§5,§8); redaction enforced + asserted over HTTP (Tasks 2,4,6 — §6); tests = ported core + supertest controller + gstack page smoke (Tasks 2,4,7 — §7); run via build+node (Task 6,7 — §8). Publish/dedupe/v2-introspection out (§9).
- **Type consistency:** gem-core `ConfigInventory`/`Gem`/`PackArtifact`/`PackSelection` (Task 2) ↔ `InventorySchema`/`PackSchema`/`PackArtifactSchema`/`PackSelectionSchema` (Task 3); `introspectConfig(dir)→ConfigInventory`, `buildPack(inv, selection, {name,createdFrom})→Gem` used identically in controller (Task 4) and tools (Task 5). Relative imports use `.js` (NodeNext).
...
- **Out of scope:** publishing Packs to a managed backend; de-duping gem-core with workflow-profiler; plugin-bundled artifacts; auth/hosting.
