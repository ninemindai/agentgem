...
- **Base branch:** off `origin/main`. **PR #55 (`feat/warm-precompute`) is MERGED** (merge commit `95f0336`, 2026-07-01), so the files this plan modifies (`src/warm/registry.ts`, `src/workflowCore.ts`, `src/insightsCore.ts`, `packages/console/src/components/WarmingPill.tsx`) are on `main`. This branch (`feat/dreaming-proposal`) was cut from `origin/main` past that merge and already contains them — execution can proceed here directly, no waiting.
...
- **Test invocation (repo-specific — confirmed in Task 1):** vitest runs COMPILED tests from `dist/`, NOT `src/`. Build first, then run the compiled file: `pnpm exec tsc -b && pnpm exec vitest run dist/dream/__tests__/<name>.test.js`. The per-task `pnpm vitest run src/...` lines below are shorthand — use this compiled form. Clean `dist/` after renaming/moving files.
...
function writeJson(path: string, value: unknown): void {
  try {
    mkdirSync(dirname(path), { recursive: true }); // inside the try, like reflectionStore
...
function cfgPath(base: string): string {
  return join(base, ".agentgem", "dream", "config.json"); // path only — no mkdir on the read path
}
...
export function setDreamEnabled(enabled: boolean, base: string = agentgemHome()): void {
  try {
    ...
    mkdirSync(dirname(p), { recursive: true }); // inside the try — never throw on dir failure
...
- Produces:
  ...
  - `reflectionToLesson(r: Reflection, root: string): DistilledLesson` — total over any `Reflection`; converts it into a `DistilledLesson` for `writeDistilledLesson`. Callers pass `entry.draft as Reflection, entry.root` only at the kind-guarded Task 5 site, so it never receives a skill draft.
...
export function slugFromReflection(r: Reflection): string {
  ...
  return `${r.kind}-${words}`.slice(0, 45); // bound the slug (feeds the dedup key), like the sibling lessonSlug
}
...
**Interfaces:**
- Consumes: `computeWorkflowAnalysis` (`src/workflowCore.js`), `dreamEnabled` (Task 2), `harvestEntries` (Task 3), `enqueueNew`/`appendDiary` (Task 1).
- Produces: `dreamRoot(root, opts): Promise<"warmed" | "hit">` where `opts` allows injecting `{ analyze, now, base, enabled }` for tests; the registry entry calls it with real deps. (v1 harvests only the `analyze` payload — DEEP; REM/insights harvesting is a deferred fast-follow, so `computeInsights` is intentionally NOT called here.)
...
import type { DistilledSkill, Reflection } from "@agentgem/insight";
import { agentgemHome, InvalidInputError } from "@agentgem/model"; // InvalidInputError lives here, not @agentback/rest
...
export class DreamController {
  ...
  async accept(input: { body: z.infer<typeof KeyBody> }): Promise<z.infer<typeof OkPathSchema>> {
    ...
    // reflectionToLesson recomputes name via slugFromReflection(r) — in production it equals
    // entry.name (harvestEntries sets both the same). Override with entry.name so the file is
    // named after the queue's canonical, already-deduped identity.
  ...
  async run(): Promise<z.infer<typeof StartedSchema>> {
    void runWarmPass({ force: true }).catch(() => {}); // fire-and-forget, best-effort; poll /api/dream/status
    return { started: true };
  }
}
```
...
- Produces: `dreamingPage: ConsolePage` (group `"observe"`, `route: "#/dreaming"`), and the `Dreaming` component with two mutually-exclusive tabs: **Review** (status + phase readout + counts + the queue) and **Diary** (v1 stub). Default tab is Review, so status and queue show on first render.
...
> The diary tab is intentionally a stub in v1 (the spec's Diary tab); a `/api/dream/diary` endpoint + list is a fast follow. The observe group already uses orders 5 (Observe), 7 (Insights), 8 (Benchmark), so `order: 9` places Dreaming at the end of the observe group without colliding.
