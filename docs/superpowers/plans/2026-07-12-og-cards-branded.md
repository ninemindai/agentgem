...
- **What actually gets edge-cached:** only `/og/card.png` sets a `Cache-Control` header, so only the (expensive-to-rasterize) PNG is written to the CF edge cache. Entity-HTML responses (`/games`, `/gems`, `/@handle`, `/skills`) are proxied **uncached by design** — they are cheap to serve (fetch shell + string-inject) and caching a full SPA shell that references hash-named JS bundles would risk serving stale HTML pointing at purged assets after a deploy. If origin load from viral links ever becomes a concern, a **future enhancement** is to have the aggregator set a SHORT `s-maxage` (e.g. 60s) on entity HTML — short enough to absorb crawler bursts without the stale-asset hazard.
- **Validation (manual, deploy-gated):** confirm live unfurls with the X, Facebook, and LinkedIn card debuggers — the same caveat the certificate card carries. Watch for edge-cache staleness of the PNG (a changed title/play-count is stale until the card's `s-maxage=3600` TTL).
