# agentgem — Gem Archive Format: a manifest + filesystem the targets consume (Design)

**Date:** 2026-06-18
**Status:** Approved design, pre-implementation
**Project:** `agentgem` (`/Users/rfeng/Projects/ninemind/agentgem`)
**Scope:** Make a Gem a **durable, portable, verifiable on-disk artifact** instead of an ephemeral in-memory object. A gem becomes a directory — a human-authored `gem.json` manifest, a generated `gem.lock` for integrity, and artifact bodies as real files — that serializes to/from the existing `Gem` type. `materialize()` and `publish` are **unchanged**: they keep consuming a `Gem`, now optionally one loaded from an archive instead of built from a live introspection. The archive is the neutral upstream side of the existing canonical→harness seam, made persistable.

---

## 0. Motivation

Today a Gem is built fresh on every operation: `introspect(~/.claude) → buildPack → materialize | publish`. Nothing is ever persisted. The JSON wire form (`PackSchema`) inlines every artifact body as an escaped string — skill content, instructions, MCP configs all live inside one blob. That makes a Gem:

- **Non-portable** — you can't save, version-in-git, share, or install a gem later; it only exists for the duration of a request bound to a live `~/.claude`.
...
```
mygem/
  gem.json                     # manifest (human-authored)
  gem.lock                     # integrity (generated)
...
```jsonc
{
  ...
  "name": "mygem",
  "version": "0.1.0",                 // semver of the gem itself (author-set)
  "createdFrom": "~/.claude",
  "artifacts": [
...
The manifest is the index: every artifact and check is referenced by `type` + `name` + `path`. `requiredSecrets` is copied verbatim from the Gem (it is already names+locations only). Skill/MCP/hook/instruction *metadata* not carried in the body file (e.g. `SkillArtifact.description`, `McpServerArtifact.transport`) is preserved — instructions/skills via their file content + manifest entry; mcp/hook via the structured JSON body (so `transport`, `event`, `matcher`, `secretRefs` round-trip exactly).
...
```jsonc
{
  ...
  "gemDigest": "sha256:…",     // hash over canonicalized manifest + sorted (path,digest) list
  "signature": null              // reserved; digest is what a signature will later cover
}
```

`gemDigest` is computed over a **canonical** serialization: the manifest re-serialized with sorted keys, concatenated with the lexicographically-sorted list of `path → sha256` entries (every file *except* `gem.lock` itself). This makes the digest stable across key ordering, whitespace, and OS line endings, and makes it the exact thing a future signature signs.

## 3. Module API — `src/gem/archive.ts` (new, pure)
...
// Compare a tree against a lock without rebuilding the Gem.
...
```ts
// src/gem/archiveTar.ts (or inline in controller) — FileTree ⇄ .tar.gz buffer
...
**UI (`src/public/index.html`):** an **"Archive"** action beside Materialize that POSTs `/api/archive` and shows the produced tree (paths click-to-view via the existing modal) plus the `gemDigest`. Out of scope for v1: a browser download of the tar (the runner/controller writes; consistent with materialize previewing rather than writing).
...
- `src/gem.controller.ts` — `@post("/archive", …)`; `archivePath` branch in materialize + publish handlers; disk write + tar.
...
- **Controller (`@agentback/testing`, temp fake `~/.claude`):** `POST /api/archive {dir, selection}` returns a tree with `gem.json` + `gem.lock` + bodies and a `gemDigest`; then `POST /api/materialize {archivePath, target:"eve"}` returns the Eve `agent/…` layout — proving archive→deployment with no live introspection. No secret value in either response.
- **Page (gstack at verify time):** load `/`, select a skill + an MCP server, click **Archive** → see `gem.json`/`gem.lock`/body paths and the digest; open `SKILL.md` in the modal.
...
3. **Publish registry generalization** — refactor `publish.ts` into a `PUBLISH_REGISTRY` of `PublishTarget { render(gem)→payload; send(payload) }`, mirroring `TARGET_REGISTRY`. Earns its keep once a 2nd network backend exists.
...
**Explicit non-goal — runtime workspace seed:** the gem archive carries agent *configuration* (skills/instructions/mcp/hooks/checks), **not** arbitrary runtime workspace files. OpenAI's sandbox `Manifest` (`entries: { "file.md": file({content}) }`) is a *different* manifest — "what's on disk when the agent wakes up," vs. the gem manifest's "what the agent *is*." (The Gem's `EvalSetup.files` is a narrow, check-only version of workspace seeding and stays scoped to checks.) Shipping gems with seed files, if ever wanted, is a separate deliberate extension.

Also out of scope: actual cryptographic **signing** (digest + reserved `signature` slot only), browser **download** of the tar, and a **gem registry / push-pull** transport (the archive is the unit a registry would later carry).

## 10. Platform fit

The archive turns a Gem from a request-scoped value into a **durable, verifiable, shareable artifact** — the interchange unit that decouples "build a gem once" from "deploy it to N harnesses, whenever, wherever." It does so with a small, pure addition (`archive.ts` + thin tar + one op) that leaves the entire target/publish machinery untouched, because the archive round-trips to the very `Gem` they already consume. Every harness reference checked during design (Eve, Flue, Codex, Claude managed agents, OpenAI sandboxes, Bedrock) maps off this one neutral artifact — which is the evidence the format is at the right altitude.
