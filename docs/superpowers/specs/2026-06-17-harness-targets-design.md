# agentgem — Harness Targets: materialize a Pack into a harness's on-disk layout (Design)

**Date:** 2026-06-17
**Status:** Approved design, pre-implementation
**Project:** `agentgem` (`/Users/rfeng/Projects/ninemind/agentgem`)
**Scope:** Make a Pack *portable across coding-agent harnesses* by adding **targets**. A target renders a normalized Pack into a specific harness's on-disk file layout via a pure `materialize(pack, target): { files, skipped }`. agentgem owns the rendering (pure functions, in-memory file tree); it writes nothing — the step-5 runner/operator writes the tree. This turns "the pack works on Claude *and* Codex" from an assertion into something the build tool can actually produce.

---

## 0. Motivation

agentgem already *ingests* from four harnesses (`~/.claude`, `~/.agents`, `~/.codex`, `~/.hermes`), normalizing their divergent formats into one artifact model tagged with a `source`. The inverse capability is missing: a Pack has no notion of which harness it is *for*, nor how to render its normalized artifacts back into that harness's layout. Without it, cross-agent portability (pressure-test assumption #2) is asserted, never produced. Materialization is also the concrete form of the step-5 runner's "install the pack into a clean agent" — the runner writes what `materialize` returns.

## 1. Design decisions (locked)

1. **A target's job is materialization** — render a Pack into a harness's on-disk layout. Compatibility is *derived* from what materialization can render (no separately-declared target list).
2. **agentgem owns a pure function → in-memory file tree** — `materialize(pack, target): MaterializeResult`. It writes nothing; the runner/operator writes the tree. Same discipline as checks (owns functions+types, runs nothing).
...
## 4. Architecture — composable conventions (`src/pack/targets.ts`)
...
The handler resolves dirs, introspects, `buildPack`s, then returns `{ target, ...materialize(pack, target), compatibility: compatibility(pack) }`. One introspect yields both the chosen target's tree and the all-targets summary. Checks/`requiredSecrets` are unaffected (they live in the pack JSON, not the harness layout).
...
- `src/pack/targets.ts` *(new)* — `TargetId`, `FileTree`, `SkippedArtifact`, `MaterializeResult`, `TargetSpec`, the shared convention renderers, `TARGET_REGISTRY`, `materialize`, `compatibility`, and the purpose-built TOML emitter (`tomlMcpServers`) + `hooksToEventMap` helper.
- `src/pack/types.ts` — re-export or define `ArtifactType` if not already exported (it is: `export type ArtifactType`). No Pack shape change.
- `src/schemas.ts` — `TargetIdSchema`, `SkippedArtifactSchema`, `MaterializeRequestSchema`, `MaterializeResponseSchema`.
- `src/pack.controller.ts` — `@post("/materialize", …)` method (MCP tool `materialize`).
...
- **`src/pack/__tests__/targets.test.ts` (unit, external fidelity):**
  ...
  - `compatibility(pack)` returns correct `{supported, skipped}` counts per target.
...
- **`agents`-target MCP**, Codex `developer_instructions` config form, AGENTS.md precedence variants (`AGENTS.override.md`, `.agents.md`), and a **per-target check matrix** (running a pack's checks once per target) — addable cells/ops later.
- **Secret rebinding / injection at install** — runner's job; materialize emits redacted configs + the pack declares `requiredSecrets`.

## 10. Platform fit

Targets make portability *producible*: a Pack built once renders into Claude, Codex, agents.md, and Hermes layouts, with an honest skip report and a derived compatibility badge. That badge and those file trees are what steps 5 (verify — install the tree into a clean agent) and 6 (publish — "runs on these harnesses") consume. agentgem's footprint stays small (one module + one op + one UI mode); everything that touches a real filesystem or a real agent remains the platform runner's.
