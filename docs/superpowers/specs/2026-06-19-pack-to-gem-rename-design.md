...
Rename the core domain concept from **`Pack`** to **`Gem`** across the whole
...
2. **MCP tool name** — `@tool("pack")` → `@tool("build_gem")` (agent-visible).
3. **On-disk archive format** — `pack.json`/`pack.lock`/`packDigest` →
...
This is a **curated rename by symbol class — not a blind `s/pack/gem/g`.**
...
| `packDigest` (field + locals) | `gemDigest` |
| `packName` / `packname` | `gemName` |
| Test fixtures `mypack` / `my_pack` | `mygem` / `my_gem` |
| lowercase `pack` used as the noun (variables, comments, prompt strings like "pack-loaded agent") | `gem` |

### Files renamed

| Old path | New path |
|---|---|
| `src/pack/` (directory) | `src/gem/` |
| `src/pack.controller.ts` | `src/gem.controller.ts` |
| `src/pack.tools.ts` | `src/gem.tools.ts` |
| `src/pack/buildPack.ts` | `src/gem/buildGem.ts` |
| `src/__tests__/pack.controller.test.ts` | `src/__tests__/gem.controller.test.ts` |
| `src/pack/__tests__/buildPack.test.ts` | `src/gem/__tests__/buildGem.test.ts` |
| `docs/superpowers/plans/2026-06-16-pack-checks.md` etc. (5 doc files matching `*pack*`) | `*-gem-*` equivalents |

Imports referencing `./pack/...`, `../pack/...`, `./buildPack` must be updated to
the new paths.

### Deliberately NOT renamed

These contain the letters "pack" but are **not** the Gem concept:
...
In `src/gem/archive.ts` (formerly `src/pack/archive.ts`):

- `MANIFEST_PATH = "pack.json"` → `"gem.json"`
- `LOCK_PATH = "pack.lock"` → `"gem.lock"`
- Lock field `packDigest` → `gemDigest`
- Error strings: `"archive missing pack.json"` → `"archive missing gem.json"`,
  `"pack.lock verification failed …"` → `"gem.lock verification failed …"`,
  and the `mismatches.push("packDigest")` → `"gemDigest"`.

No backward-compatible reading of legacy `pack.json`/`pack.lock`.

## MCP tool change

In `src/gem.tools.ts`:

- `@tool("pack", …)` → `@tool("build_gem", …)`
- default name fallback `input.name ?? "pack"` → `?? "gem"`
- The sibling `@tool("inventory", …)` is unchanged.

## Docs + memory

- Rename the 5 `*pack*` files under `docs/superpowers/plans/` and
  `docs/superpowers/specs/` to `*gem*` and update their prose.
- Update the auto-memory: rename/rewrite `pack-archive-format-spec.md`
  (now describing `gem.json`/`gem.lock`/`gemDigest`) and refresh the
  `MEMORY.md` index line. Other memory files that mention "Pack" in prose
...
avoid partial-match damage, e.g. replace `PackArtifactSchema` before `Pack`),
