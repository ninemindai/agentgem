...
and local testbeds and runs. For the conceptual "why," read [Concepts](concepts.md)
first.
...
4. **Distribution** — the neutral Gem feeds targets (materialize), the marketplace, and
   local testbeds/runs. See [distribution](#distribution) below.
...
The Gem is a neutral source, consumed by targets and the marketplace, plus local
testbeds and runs.
...
- **Targets** (`@agentgem/model`) — `materialize(gem, target)` runs per-artifact renderers and a
  ...
  [Targets](targets.md).
- **Marketplace** (`@agentgem/distribute`, `@agentgem/contract`) — publish to and install
  from the hosted marketplace at [app.agentgem.ai](https://app.agentgem.ai), over the same
  archive format. See [Sharing & identity](sharing.md).
- **Testbed & Run** (`@agentgem/testbed`, `@agentgem/run`) — install a Gem into a local `.claude`/`.codex`/
  `.hermes` testbed, or run a materialized project locally. See
  [Testbed & run](testbed-and-run.md).
...
- [Targets](targets.md) · [Testbed & run](testbed-and-run.md)
