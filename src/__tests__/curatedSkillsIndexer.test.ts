...
describe("indexCuratedSkills", () => {
  it("indexes a skills-layout source: stars + skill list upserted", async () => {
  ...
  it("indexes an agency-layout source via divisions+agents", async () => {
    const db = await makeTestDb();
    const r = await indexCuratedSkills(db, { sources: [AGENCY_SOURCE], http: fakeHttp() });
    expect(r).toEqual({ sources: 1, skills: 1 });
    const rows = await popularSkills(db, { limit: 10 });
    expect(rows[0]).toMatchObject({ sourceId: "agency-repo", division: "engineering", stars: 7, homepage: null });
  });
