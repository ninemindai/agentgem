...
export interface ToolDeps {
  ...
  // Consent gate for network contribution: the interactive publish path ingests to the hosted
  // benchmark ONLY when the producer opted in (benchmarkContribute). Injected as () => false in tests.
...
export async function signAndPublishTool(
  ...
  // benchmark ONLY when the producer opted in (same toggle as bulk contribution). Off by
  // default, so publishing never silently POSTs to the network (and tests inject () => false).
  const ingest = (deps.contributeEnabled ?? benchmarkContribute)()
    ? await postAttestation({ attestation: signed, token: input.token, http: deps.ingestHttp })
