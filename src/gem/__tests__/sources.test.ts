...
describe("SourceSpec built-ins", () => {
  it("registers claude, cline, codex, continue, cursor, gemini, and atif", () => {
    expect(BUILTIN_SOURCES.map((s) => s.id).sort()).toEqual(["atif", "claude", "cline", "codex", "continue", "cursor", "gemini"]);
  });
