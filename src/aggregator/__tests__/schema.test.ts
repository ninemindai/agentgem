...
describe("schema/testDb", () => {
  it("creates the schema and runs drizzle queries on pglite", async () => {
    ...
    expect((t.rows as { table_name: string }[]).map((x) => x.table_name)).toEqual(["account", "account_bindings", "account_scopes", "accounts", "api_keys", "app_installations", "attestations", "catalog_gems", "connect_states", "curated_skills", "game_plays", "gem_adoptions", "gem_archives", "gem_group_shares", "group_invites", "group_members", "groups", "handoff_codes", "ingredients", "model_outcomes", "org_members", "org_settings", "passkey", "pending_account_links", "producers", "review_messages", "review_requests", "review_seen", "reviews", "session", "share_cards", "stars", "usage_day_models", "usage_days", "usage_edges", "user", "verification"]);
  });
