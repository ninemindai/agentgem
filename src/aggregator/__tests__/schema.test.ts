...
describe("schema/testDb", () => {
  it("creates the schema and runs drizzle queries on pglite", async () => {
    ...
    expect((t.rows as { table_name: string }[]).map((x) => x.table_name)).toEqual(["account_bindings", "account_scopes", "accounts", "api_keys", "attestations", "catalog_gems", "gem_adoptions", "ingredients", "model_outcomes", "producers", "share_cards", "stars", "usage_edges", "web_sessions"]);
  });
});
