...
describe("POST /api/aggregator/sweep", () => {
  ...
  it("dry-run (apply omitted) reports but changes nothing", async () => {
    ...
    expect(res).toEqual({ ok: true, report: { clustersFound: 1, attestationsQuarantined: 3, producersFlagged: 3, adoptionsQuarantined: 0, adoptionGemsFlagged: 0, adoptionProducersFlagged: 0, staleReviewRequestsSwept: 0, dryRun: true } });
  ...
  it("apply:true quarantines", async () => {
    ...
    expect(res).toEqual({ ok: true, report: { clustersFound: 1, attestationsQuarantined: 3, producersFlagged: 3, adoptionsQuarantined: 0, adoptionGemsFlagged: 0, adoptionProducersFlagged: 0, staleReviewRequestsSwept: 0, dryRun: false } });
