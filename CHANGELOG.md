...
"active" hours that could exceed 24 in a single day, because a session left open
contributed its whole wall-clock span. SETUP scored tooling breadth against a cap of
10, so every operator past ten skills scored identically. Both were visible on screen
for months and neither looked broken.

The release also fixes the login 0.10.5 was written to fix. That entry describes the
device flow as polling "for five minutes and then throws", and treats the five minutes
as correct. It is not: GitHub honours a device code for about 900 seconds, so the CLI
was declaring a timeout on a code with ten minutes of life left.

### Fixed

- **The device flow no longer gives up on a live code.** GitHub's `expires_in` (~900s)
  was discarded in favour of 60 attempts × 5s, so a user who took six minutes was told
  the flow timed out while the code was still good. The old budget was accidentally
  interval-dependent too — 60 attempts is five minutes at `interval=5` and ten at
  `interval=10`. It is a time budget now, spent to the code's real expiry.

  The budget deliberately is not raised everywhere. The console's `POST /bind/complete`
  polls inside the HTTP request, where a 15-minute budget means a 15-minute request that
  dies in a proxy with the answer lost. It stays at 300s; only the CLIs wait longer.
- **Overview's hours no longer count idle time.** Each session contributed its full
  first-event → last-event span, so one left open overnight reported the whole night,
  and a real payload showed 67.1h in a single day. Each gap between consecutive records
  is now capped at 5 minutes, so a long idle stretch contributes at most that while a
  genuine long build still counts. Sessions are still summed rather than merged: with
  parallel agents the total is compute-hours, and can legitimately pass 24h/day.
- **46 test files were never running.** The vitest glob missed every package outside the
  root — 238 tests, including the console, marketplace and insight suites. They run now,
  and the release candidate is 3,000 tests green.
- **An unreadable artifact-outcomes schema is refused rather than stamped**, instead of
  being written over with a version claim it could not verify.
- **Scan and parse caches carry new versions**, so stats computed before engaged time
  existed are recomputed rather than served.

### Changed

- **SETUP scores breadth on a log curve, not a ratio capped at 10.** `min(1, count/cap)`
  priced the step from 3 skills to 10 the same as 30 to 37, and the caps (10 skills, 5
  subagents) were low enough that most real users sat at 100% on both terms — 30% of the
  score saying nothing. It is `log1p(count)/log1p(cap)` now, with caps of 25 and 8.

  **This rescores everyone**, which is intended. The low end gains (one skill goes from
  0.100 to 0.213 of the term) because the first tools should visibly pay; the profile
  sitting exactly on both old caps loses about 7 points. Owning tools you never reach for
  still scores badly, since coverage remains 70% of SETUP. Already-published cards keep
  their original numbers — a card is immutable once shipped — so `GEMIT_SHARE_VERSION` is
  1.1.0 to record which formula produced a given card. The renderer did not change.
- **The Overview and Reveal figure is labeled "engaged", not "active"**, so the number
  reads as what it now measures. Both surfaces moved together, so they cannot disagree.
- **The org-usage reporter reports engaged time**, consistent with Overview.

### Added

- **The CLI says it is still waiting.** 0.10.5 made it open the browser for you, but the
  wait itself stayed silent, which is how the reporter wandered off in the first place.
  Both `gemit` and `agentgem bind` now print a line at 30 seconds and then every two
  minutes: about eight lines across a full wait. Every line repeats the code, because by
  then the original instructions have scrolled away; only the first repeats the URL.
  Nothing prints once the budget is spent, so it never announces "30s left" and then dies.
- **`ScrubbedStep.verbDetail`**, so a script-driven procedure can be named rather than
  detected and left undescribed.

## [desktop-v0.11.0] — desktop app — 2026-07-29

### Changed

- **Overview and Reveal show engaged time**, relabeled from "active". This is the release's
  most visible desktop change: the ledger's hours drop to something believable.
- **The gemit screen scores SETUP on the new curve**, so a report generated in the app
  matches one generated by the CLI.

Desktop's own GitHub connect flow is deliberately untouched. It goes through
`POST /bind/complete`, which keeps the shorter in-request budget for the reason given
above, and it has an in-flight indicator rather than a printed heartbeat.

Embeds everything in core 0.11.0.
