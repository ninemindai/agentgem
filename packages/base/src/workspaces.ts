...
export interface WorkspaceSummary {
  ...
  gemName: string;
  version: string;
  artifactCounts: { skill: number; mcp_server: number; instructions: number; hook: number; subagent: number; game: number };
...
function countArtifacts(entries: { type: string }[]): WorkspaceSummary["artifactCounts"] {
  const c = { skill: 0, mcp_server: 0, instructions: 0, hook: 0, subagent: 0, game: 0 };
