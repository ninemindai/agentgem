...
interface Entry { sessionId: string; token: string; html: string; ts: number }
function readAll(): Entry[] {
  try { const j = JSON.parse(readFileSync(cachePath(), "utf8")); return Array.isArray(j) ? j : []; } catch { return []; }
}

/** Cached entry (html + write timestamp) for (sessionId, token), or null on miss/stale. */
export function readDashboardCacheEntry(sessionId: string, token: string): { html: string; ts: number } | null {
  const e = readAll().find((x) => x.sessionId === sessionId && x.token === token);
  return e ? { html: e.html, ts: e.ts } : null;
}

/** Store (sessionId, token) → html, replacing any prior entry for the session. Capped + best-effort. */
export function writeDashboardCache(sessionId: string, token: string, html: string, nowMs: number): void {
  try {
    const all = readAll().filter((x) => x.sessionId !== sessionId);
    all.push({ sessionId, token, html, ts: nowMs });
