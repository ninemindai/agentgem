...
export function StoneRating({ cut, grade, stars, installs, verifiedInstalls }: { cut?: string; grade?: number; stars: number; installs?: number; verifiedInstalls?: number }) {
  const m = cutMeta(cut);
  const fg = m?.fg ?? NEUTRAL.fg;
  const bg = m?.bg ?? NEUTRAL.bg;
  const n = stoneRating(grade, stars, installs ?? 0);
  const diamond = isDiamond(grade, stars, verifiedInstalls ?? 0);
