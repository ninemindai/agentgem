...
describe("pages registry", () => {
  it("registers curate + your-gems with unique ids, sortable by order", () => {
    ...
    expect(ordered.map((p) => p.id)).toEqual(["observe", "optimize", "mine", "chat", "insights", "benchmark", "watch", "dreaming", "curate", "settings", "materialize", "your-gems", "publish", "get-gems", "deploy", "received"]);
  });
