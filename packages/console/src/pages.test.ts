...
describe("pages registry", () => {
  it("registers curate + your-gems with unique ids, sortable by order", () => {
    ...
    expect(ordered.map((p) => p.id)).toEqual(["observe", "optimize", "mine", "insights", "benchmark", "dreaming", "curate", "settings", "chat", "materialize", "your-gems", "publish", "get-gems", "deploy", "received"]);
  });
