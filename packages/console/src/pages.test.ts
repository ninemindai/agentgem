...
describe("pages registry", () => {
  it("registers curate + your-gems with unique ids, sortable by order", () => {
    ...
    expect(ordered.map((p) => p.id)).toEqual(["observe", "optimize", "mine", "chat", "insights", "benchmark", "watch", "dreaming", "rubrics", "curate", "settings", "materialize", "your-gems", "publish", "rubric-library", "sources", "get-gems", "deploy", "received"]);
  });
  ...
  it("assigns each page to a sidebar group", () => {
    ...
    expect(g.observe.map((p) => p.id)).toEqual(["observe", "optimize", "mine", "chat", "insights", "benchmark", "watch", "dreaming", "rubrics"]);
    expect(g.build.map((p) => p.id)).toEqual(["curate", "materialize", "deploy"]);
    expect(g.library.map((p) => p.id)).toEqual(["your-gems", "publish", "rubric-library", "sources", "get-gems", "received"]);
