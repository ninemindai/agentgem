...
describe("pages registry", () => {
  it("registers curate + your-gems with unique ids, sortable by order", () => {
    ...
    expect(ordered.map((p) => p.id)).toEqual(["observe", "watch", "optimize", "mine", "insights", "benchmark", "dreaming", "curate", "settings", "chat", "materialize", "your-gems", "publish", "get-gems", "deploy", "received"]);
  });
  ...
  it("assigns each page to a sidebar group", () => {
    ...
    expect(g.observe.map((p) => p.id)).toEqual(["observe", "watch", "optimize", "mine", "insights", "benchmark", "dreaming"]);
    expect(g.build.map((p) => p.id)).toEqual(["curate", "chat", "materialize", "deploy"]);
