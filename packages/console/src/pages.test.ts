...
describe("pages registry", () => {
  it("registers curate + your-gems with unique ids, sortable by order", () => {
    ...
    expect(ordered.map((p) => p.id)).toEqual(["observe", "mine", "curate", "settings", "materialize", "your-gems", "get-gems", "deploy", "received"]);
  });
  ...
  it("assigns each page to a sidebar group", () => {
    ...
    expect(g.observe.map((p) => p.id)).toEqual(["observe", "mine"]);
