...
// the same key; two different inputs MUST NOT. Object keys are sorted recursively (so JSON.stringify of the
// body is itself sorted and stable); arrays keep their given order (order is meaningful there). Only JSON
...
// Recursively normalize into a new value whose object keys are inserted in sorted order (so the default
// JSON.stringify emits them sorted) and whose only shapes are JSON scalars/arrays/plain objects. `undefined`
...
export function mcpIdentity(server: string, tool: string, input?: unknown): { key: string; body: unknown } {
  ...
  const key = `server:${server}|tool:${tool}|input:${JSON.stringify(body)}`;
  return { key, body };
}
