...
description: Use when rendering an AgentGem analysis report — a self-contained HTML readout built from a rubric evaluation's FACTS JSON (factors, findings, hygiene scores, per-session rows). Covers the facts-only rule, the embedded-JSON anti-hallucination seam, theming, and the editorial composition contract.
---
